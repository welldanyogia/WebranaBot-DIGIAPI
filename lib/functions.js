...
const spinner = { 
  ...
  "frames": [
	...
	//APIGAMES
      ...
      async function depositRequest(nominal) {
        try {
          const endpoint = `https://v1.apigames.id/v2/deposit-get?merchant=${conf.merchantId}&nominal=${nominal}&secret=${conf.secretKey}`;
          const response = await axios.get(endpoint);
          const depositData = response.data;
      
          // Lakukan penanganan response sesuai kebutuhan Anda
          if (depositData.status === 1) {
            const response ={
              ...
              data:{
                admin:depositData.data.admin,
                expired:depositData.data.expired,
                id:depositData.data.id,
                kode_unik:depositData.data.kode_unik,
                nominal:depositData.data.nominal,
                rekening:depositData.data.rekening,
                total_transfer:depositData.data.total_transfer
              },
              ts:depositData.ts
            }
            console.log('Permintaan deposit berhasil:');
            console.log('ID Deposit:', response.data.id);
            console.log('Nominal:', response.data.nominal);
            console.log('Rekening:', response.data.rekening);
            console.log('Total Transfer:', response.data.total_transfer);
            return response
          } else {
            const responseTextFailed = `Permintaan deposit gagal: ${depositData.error_msg}`
            console.log(responseTextFailed);
            // x.reply(responseTextFailed)
            return response
          }
        } catch (error) {
          console.error('Terjadi kesalahan dalam permintaan deposit:', error.message);
        }
      }
      ...
      async function cekKoneksiHiggs() {
        const engine = 'higgs';
        const signature = createSignatureAPI(conf.merchantId, conf.secretKey)
        const url = `https://v1.apigames.id/merchant/${conf.merchantId}/cek-koneksi?engine=${engine}&signature=${signature}`;
          ...
          try {
            ...
            if (responseData.status === 1 && responseData.rc === 0) {
              const dataHiggs = responseData.data.data;
              const connectionInfo = {
                message: responseData.message,
                coin_1m: dataHiggs[0].coin_1m,
                coin_60m: dataHiggs[0].coin_60m,
                coin_200m: dataHiggs[0].coin_200m,
                coin_400m: dataHiggs[0].coin_400m,
                coin_1b: dataHiggs[0].coin_1b,
                coin_2b: dataHiggs[0].coin_2b,
      ...
      async function cekKoneksiUnipinBrazil() {
      ...
      async function cekKoneksiUnipinID() {
        const engine = 'unipin';
        const signature = createSignatureAPI(conf.merchantId, conf.secretKey)
        const url = `https://v1.apigames.id/merchant/${conf.merchantId}/cek-koneksi?engine=${engine}&signature=${signature}`;
        ...
        try {
          ...
          if (responseData.status === 1 && responseData.rc === 200) {
            const data = responseData.data;
            const koneksiUnipinID = {
              name: data[0].name,
              email: data[0].email,
              up_balance: data[0].up_balance,
              uc_balance: data[0].uc_balance
            };
            return koneksiUnipinID;
          } else {
            throw new Error(responseData.error_msg);
          }
        } catch (error) {
      ...
      async function cekKoneksiUnipinMY() {
        const engine = 'unipinmy';
        const signature = createSignatureAPI(conf.merchantId, conf.secretKey)
        const url = `https://v1.apigames.id/merchant/${conf.merchantId}/cek-koneksi?engine=${engine}&signature=${signature}`;
      ...
      async function cekKoneksiUnipinPH() {
        const engine = 'unipinph';
        const signature = createSignatureAPI(conf.merchantId, conf.secretKey)
        const url = `https://v1.apigames.id/merchant/${conf.merchantId}/cek-koneksi?engine=${engine}&signature=${signature}`;
...
module.exports = {
  Spam: {
    ...
    createSignatureAPI,getInfoAkun,cekKoneksiSmileOne,cekKoneksiHiggs,cekKoneksiKiosgamer,cekKoneksiUnipinID,cekKoneksiUnipinBrazil,cekKoneksiUnipinMY,cekKoneksiUnipinPH,beliProduk,cekStatusTransaksi,depositRequest}