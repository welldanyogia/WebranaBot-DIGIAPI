...
const { createSignature,orderDigi,searchPriceByCode,getPriceList,generateRefId,checkTransaction,cekAkun,createSignatureAPI,getInfoAkun,cekKoneksiSmileOne, cekKoneksiKiosgamer, cekKoneksiUnipinBrazil,beliProduk,cekStatusTransaksi,orderHdi} = require('./functions')
      ...
      case 'h':
        {
          const refId = generateRefId(10);
            ...
            const tujuan = parameter.split(' ')[1];
              ...
              orderHdi(refId, produk, tujuan)
              .then(async hasil => {
                let responseText = `*≽  DETAIL TRANSAKSI ≼*\n\n`
                    responseText += `⭔TRX ID : ${hasil.trx_id}\n`
                    responseText += `⭔REF ID : ${hasil.ref_id}\n`
                    responseText += `⭔Produk : ${hasil.product_detail.name}\n`
                    responseText += `⭔Tujuan : ${hasil.destination}\n`
                    responseText += `⭔Pesan  : ${hasil.message}\n`
                    responseText += `⭔Status : ${hasil.status}\n`
                x.reply(responseText)
                if (hasil.status == 'Pending') {
                  await delay(3000)
                  let cek = await cekStatusTransaksi(refId)
                  ...
                  let responseTextChecked = `*≽  DETAIL TRANSAKSI ≼*\n\n`
                    responseTextChecked += `⭔TRX ID : ${cek.trx_id}\n`
                    responseTextChecked += `⭔REF ID : ${cek.ref_id}\n`
                    responseTextChecked += `⭔Produk : ${cek.product_detail.name}\n`
                    responseTextChecked += `⭔Tujuan : ${cek.destination}\n`
                    responseTextChecked += `⭔Pesan  : ${cek.message}\n`
                    responseTextChecked += `⭔Status : ${cek.status}\n`
                    x.reply(responseTextChecked)
                }
                console.log(hasil);
              })
