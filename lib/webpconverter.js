    ...
    FileType.fromBuffer(bufferImage)
      .then((response) => {
        try {
          ...
          exec(
            `cwebp -q 50 ${pathFile} -o ${pathFile}.webp`,
            (error, stdout, stderr) => {
...
function gifToWebp(bufferImage) {
  return new Promise((resolve, reject) => {
    try {
      const pathFile =
        ".temp/" + Math.floor(Math.random() * 1000000 + 1) + ".gif";
      fs.writeFileSync(pathFile, bufferImage);

      exec(
        `gif2webp ${pathFile} -o ${pathFile}.webp`,
        (error, stdout, stderr) => {
          if (!fs.existsSync(pathFile + ".webp")) {
            reject(new Error("failed convert file!"));
            fs.unlinkSync(pathFile);
...
function webpToVideo(bufferImage) {
  return new Promise((resolve, reject) => {
    try {
      const pathFile =
        ".temp/" + Math.floor(Math.random() * 1000000 + 1) + ".webp";
      fs.writeFileSync(pathFile, bufferImage);
